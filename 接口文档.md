...
本文档用于说明 `beauty-console` 后台接口、请求参数、返回字段和主要业务规则。文档位于仓库根目录，后端命令默认从 `beauty-console` 目录执行。
...
如果不重置数据库，可执行 `beauty-console/store-server/src/main/resources/sql/patch_test_role_accounts.sql` 补齐这些账号。
...
```http
POST /admin/service-orders/idempotency-token
```
...
| 字段 | 类型 | 说明 |
|---|---|---|
| data | String | 当前登录用户可使用的 UUID 幂等令牌 |

说明：令牌默认 10 分钟内可开始使用。新增订单或预约转订单时必须在 `Idempotency-Key` 请求头携带该 UUID；同一令牌成功重试会返回第一次生成的订单 ID。
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| Idempotency-Key | String(UUID) | 是 | 通过 11.3 接口获取的下单幂等令牌 |
...
| 字段 | 类型 | 说明 |
|---|---|---|
| data | Long | 新增订单ID；同一令牌成功重试时返回原订单ID |
...
| 字段 | 类型 | 说明 |
|---|---|---|
| data | Long | 生成的订单ID；同一预约或同一令牌重试时返回原订单ID |
...
| `/admin/permissions/roles/{roleId}` | GET | 查询指定角色默认权限 |
| `/admin/permissions/roles/{roleId}` | PUT | 保存指定角色默认权限 |
| `/admin/users/{id}/permissions` | GET | 查询指定账号当前生效权限 |
| `/admin/users/{id}/permissions` | PUT | 保存指定账号权限点 |
| `/admin/users/{id}/reset-password` | PATCH | 把指定账号密码重置为 `123456` |
...
- 角色默认权限接口使用同样的 `permissionCodes` 数组；普通角色不能突破后端定义的角色权限上限。
- 修改账号角色时会删除旧的账号级覆盖，使账号重新继承新角色默认权限。
- 密码重置接口只允许超级管理员和店长调用，返回成功后目标账号使用 `123456` 登录。

新增耗材脚本：`beauty-console/store-server/src/main/resources/sql/patch_inventory_consumables.sql`。
项目耗材脚本：`beauty-console/store-server/src/main/resources/sql/patch_service_project_inventory.sql`。
权限点脚本：`beauty-console/store-server/src/main/resources/sql/patch_permission_points.sql`。
当前权限脚本包含 59 个权限点；新增耗材为 15 条，项目耗材关系为 21 条。

## 本轮接口行为补充：编号、审计、状态流转、删除保护和联调测试
...
| 业务 | 字段 | 规则 | 数据库约束 |
|---|---|---|---|
| 预约 | appointmentNo | `APT` + IdWorker | `uk_appointment_no` |
| 服务订单 | orderNo | `ORD` + IdWorker | `uk_service_order_no` |
| 收款流水 | paymentNo | `PAY` + IdWorker | `uk_payment_no` |

补丁脚本：

```text
beauty-console/store-server/src/main/resources/sql/patch_audit_and_unique_numbers.sql
```

### 操作审计

关键写操作会自动写入 `operation_audit_log`：

| 字段 | 说明 |
|---|---|
| operatorUserId | 当前登录账号 ID |
| operatorStaffId | 当前登录员工 ID |
| actionType | CREATE、UPDATE、DELETE、STATUS、CONFIRM、COMPLETE、CANCEL、VOID 等 |
| targetType | CUSTOMER、STAFF、SERVICE_PROJECT、INVENTORY_SKU、ORDER、APPOINTMENT、PAYMENT_RECORD 等 |
| targetId | 操作目标 ID |
| createTime | 操作时间 |
...
以下删除接口会先检查业务引用，存在引用时返回业务失败：

| 资源 | 禁止删除条件 |
|---|---|
| 客户 | 已有关联预约或订单 |
| 员工 | 已有关联账号、预约、预约项目或订单项目 |
| 服务项目 | 已被预约项目、订单项目或项目耗材关系引用 |
| 库存物品 | 已存在库存流水或项目耗材关系 |
| 预约 | 已生成服务订单 |
| 服务订单 | 已存在收款记录或库存流水 |
...
| 接口 | 方法 | 说明 |
|---|---|---|
| `/admin/appointments/{id}/confirm` | PATCH | 确认预约 |
| `/admin/appointments/{id}/complete` | PATCH | 完成预约 |
| `/admin/appointments/{id}/cancel` | PATCH | 取消预约 |

重复确认、取消后完成、完成后取消等非法流转会返回业务失败。

### 收款并发处理

新增收款和作废收款统一由 `PaymentRecordService` 在事务内处理：

- 锁定服务订单行后重新计算 `paidAmount`、`debtAmount`、`payStatus`。
- 作废收款会锁定收款流水，避免重复作废。
- 超过订单应收金额的收款会被拒绝。
...
| 接口 | 方法 | 说明 |
|---|---|---|
| `/admin/payment-records` | POST | 新增收款记录并反写订单金额 |
| `/admin/payment-records/{id}/void` | PATCH | 作废收款记录并回滚订单金额 |
...
真实 MySQL + Redis 冒烟测试默认跳过，需要显式开启：
...
```text
STORE_REAL_DB_URL
STORE_REAL_DB_USERNAME
STORE_REAL_DB_PASSWORD
STORE_REAL_REDIS_HOST
STORE_REAL_REDIS_PORT
STORE_REAL_REDIS_PASSWORD
STORE_REAL_REDIS_DATABASE
```

2026-07-14 本地验证结果：

```text
mvn -pl store-server -am test          -> 52 total, 51 passed, 1 real-environment test skipped
npm run build (beauty-console-web)     -> build passed, only legacy dependency/asset-size warnings
npm run lint (beauty-console-web)      -> 11 existing errors, mainly generated iconfont.js
npm run test:unit -- --runInBand       -> 1 suite passed, Breadcrumb suite missing route mock
```
...
```http
GET /admin/dashboard/overview?refresh=false
```
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| refresh | Boolean | 否 | 默认 false；true 表示绕过旧值、重新计算并写入缓存 |

返回 `data`：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| customerTotal | Long | 当前账号有客户查看权限时返回客户总数 |
| appointmentTotal | Long | 当前账号有预约查看权限时返回预约总数 |
| orderTotal | Long | 当前账号有订单查看权限时返回订单总数 |
| inventoryTotal | Long | 当前账号有库存查看权限时返回库存耗材总数 |
| pendingOrders | Array<ServiceOrderVO> | 最近 5 条待服务订单摘要，不加载明细和支付记录 |
| appointments | Array<AppointmentVO> | 最近 5 条预约摘要 |
| lowStockItems | Array<InventorySkuVO> | 库存量不高于安全库存的前 5 条耗材 |

结果使用 Redis 缓存 30 秒，key 设计为 `dashboard:overview::{用户ID}:{角色ID}`，避免不同用户和角色共用同一份权限过滤结果。手动刷新按钮传 `refresh=true`。权限被修改时会清理控制台、权限码和权限规则缓存。

当前 OpenAPI 共 78 个业务接口、14 个 Controller。完整 Maven 回归共 85 项：84 项通过，1 项真实外部环境测试按设计跳过。
